...
If the former, the coordinates need to be in order (i.e. xvals should be organized
from west to east, yvals from south to north).

If the locations are gridded, the netCDF will be created with a lat and long dimension.

If the locations are not gridded, the netCDF will be created with an automatically
generated *site* dimension, and lat and long variables affiliated with each site,
similar to [this example](http://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html#_orthogonal_multidimensional_array_representation_of_time_series).

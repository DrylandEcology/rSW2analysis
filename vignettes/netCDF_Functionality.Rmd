...
in the western hemisphere, xvals should be organized from west to east, yvals from south to north.
...
Use CMIP6 standard variable names where available.The [day](https://github.com/PCMDI/cmip6-cmor-tables/blob/master/Tables/CMIP6_day.json)
...
Additionally, we strongly recommend defining the CRS in the crs_wkt format, which
...
The *wkt* function in the raster package can be used to generate wkts.
...
locationsSP <- sp::SpatialPoints(locations)
sp::proj4string(locationsSP) <- sp::CRS(crs_attributes$proj)
WKTex <- raster::wkt(locationsSP)

crs_attributes <- c(crs_attributes, crs_wkt = WKTex)
...
the lat/long locations, and the area that each cell in the grid represents.
...
**var_LmonOrAmon_timestep_Project_SOILWAT2_GCM_RCP_variantLabel_gn_startT-endT.nc**

where:

* var – Shorthand name for variable that is contained in files (i.e. ppt for precipitation, WDD for wet degree days)
* LmonOrAmon – Land (L) or atmospheric (A) data
...
* variantLabel – Variant of the GCM used (e.g. r1i1p1f1)
...
identical(lat, sort(unique(locations$Y_WGS84), decreasing = TRUE))
...
depthVals <- c(ncvar_get(tmp, varid = "depth"))
identical(depthVals, vertical_attributes[['vals']])

depthBnds <- c(ncvar_get(tmp, varid = "depth_bnds"))
